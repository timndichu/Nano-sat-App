...
class AccelerometerCharts extends StatefulWidget {

  @override
  _AccelerometerChartsState createState() => _AccelerometerChartsState();
}

class _AccelerometerChartsState extends State<AccelerometerCharts> {
  ...
  Widget build(BuildContext context) {
    Widget roomOneheader() => Ink(
      ...
      return SfCartesianChart(
          title: ChartTitle(text: 'Live Accelerometer Readings'),
                ...
                builder: (context, model, child) {
                  Widget content = Center(
                      child: Text(
                          'Error fetching data. Check your Internet connection'));
                  if (model.today) {
                    if (model.isTodayAccelerometerLoading) {
                      print(model.todayAccelerometer);
                      content = ShimmerLoader();
                    } else if ((model.todayAccelerometer.length == 0 &&
                        !model.isTodayAccelerometerLoading)) {
                      content = Center(
                          child: Padding(
                        padding: const EdgeInsets.all(8.0),
                        ...
                        !model.isTodayAccelerometerLoading)) {
                      content = GraphWidget(
                          readings: model.todayAccelerometer,
                          label: 'Today',
                           title: 'Today Accelerometer Readings',
                          dateType: 'Today');
                    }
                  } else {
                    content = Container();
                ...
                builder: (context, model, child) {
                  Widget content = Center(
                      child: Text(
                          'Error fetching data. Check your Internet connection'));
                  if (model.yesterday) {
                    if (model.isYesterdayAccelerometerLoading) {
                      print(model.yesterdayAccelerometer);
                      content = ShimmerLoader();
                    } else if ((model.yesterdayAccelerometer.length == 0 &&
                        !model.isYesterdayAccelerometerLoading)) {
                      content = Center(
                          child: Padding(
                        padding: const EdgeInsets.all(8.0),
                      ...
                      content = yesterday
                          ? GraphWidget(
                              readings: model.yesterdayAccelerometer,
                              label: 'Yesterday',
                               title: 'Yesterday Accelerometer Readings',
                              dateType: 'Yesterday')
                          : Container();
                    }
                  } else {
                ...
                builder: (context, model, child) {
                  Widget content = Center(
                      child: Text(
                          'Error fetching data. Check your Internet connection'));
                  if (model.pastWeek) {
                    if (model.isPastWeekAccelerometerLoading) {
                      print(model.pastWeekAccelerometer);
                      content = ShimmerLoader();
                    } else if ((model.pastWeekAccelerometer.length == 0 &&
                        !model.isPastWeekAccelerometerLoading)) {
                      content = Center(
                          child: Padding(
                        padding: const EdgeInsets.all(8.0),
                        ...
                        !model.isPastWeekAccelerometerLoading)) {
                      content = GraphWidget(
                        readings: model.pastWeekAccelerometer,
                        label: 'Past Week',
                         title: 'Past Week Accelerometer Readings',
                        dateType: 'Past Week',
                      );
                    }
                  } else {
                ...
                builder: (context, model, child) {
                  Widget content = Center(
                      child: Text(
                          'Error fetching data. Check your Internet connection'));
                  if (model.pastMonth) {
                    if (model.isPastMonthAccelerometerLoading) {
                      print(model.pastMonthAccelerometer);
                      content = ShimmerLoader();
                    } else if ((model.pastMonthAccelerometer.length == 0 &&
                        !model.isPastMonthAccelerometerLoading)) {
                      content = Center(
                          child: Padding(
                        padding: const EdgeInsets.all(8.0),
                        ...
                        !model.isPastMonthAccelerometerLoading)) {
                      content = GraphWidget(
                          readings: model.pastMonthAccelerometer,
                          label: 'Past Month',
                        title: 'Past Month Accelerometer Readings',
                          dateType: 'Past Month');
                    }
                  } else {
                    content = Container();
                ...
                InkWell(
                  ...
                  onTap: () {
                    Navigator.push(
                        context,
                        CupertinoPageRoute(
                            builder: (context) => ExpandedAccelerometer(
                              title: title,
...
class _SampleViewState extends State<SampleView> {
  ...
  SfCartesianChart _buildDefaultLineChart() {
    return SfCartesianChart(
      key: GlobalKey(),
      plotAreaBorderWidth: 0,
      title: ChartTitle(text: 'Accelerometer Readings'),
         tooltipBehavior: TooltipBehavior(enable: true),
          legend: Legend(
            isVisible: true,
            overflowMode: LegendItemOverflowMode.wrap,
...
//Expanded Accelerometer

class ExpandedAccelerometer extends StatefulWidget {
  final List<SensorReading> readings;
  final String title;
  final String dateType;
  ExpandedAccelerometer({this.readings, this.dateType, this.title});
  @override
  _ExpandedAccelerometerState createState() => _ExpandedAccelerometerState();
}

class _ExpandedAccelerometerState extends State<ExpandedAccelerometer> {
  TrackballBehavior _trackballBehavior;
    ChartSeriesController _chartSeriesController;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: Text('Accelerometer Readings'),
          actions: [
            IconButton(
              onPressed: () {
