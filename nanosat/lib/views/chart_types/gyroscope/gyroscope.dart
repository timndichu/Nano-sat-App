                ...
                builder: (context, model, child) {
                  Widget content = Center(
                      child: Text(
                          'Error fetching data. Check your Internet connection'));
                  if (model.today) {
                    if (model.isTodayGyroscopeLoading) {
                      print(model.todayGyroscope);
                      content = ShimmerLoader();
                    } else if ((model.todayGyroscope.length == 0 &&
                        !model.isTodayGyroscopeLoading)) {
                      content = Center(
                          child: Padding(
                        padding: const EdgeInsets.all(8.0),
                        ...
                        !model.isTodayGyroscopeLoading)) {
                      content = GraphWidget(
                          readings: model.todayGyroscope,
                          label: 'Today',
                           title: 'Today Gyroscope Readings',
                          dateType: 'Today');
                    }
                  } else {
                    content = Container();
                ...
                builder: (context, model, child) {
                  Widget content = Center(
                      child: Text(
                          'Error fetching data. Check your Internet connection'));
                  if (model.yesterday) {
                    if (model.isYesterdayGyroscopeLoading) {
                      print(model.yesterdayGyroscope);
                      content = ShimmerLoader();
                    } else if ((model.yesterdayGyroscope.length == 0 &&
                        !model.isYesterdayGyroscopeLoading)) {
                      content = Center(
                          child: Padding(
                        padding: const EdgeInsets.all(8.0),
                      ...
                      content = yesterday
                          ? GraphWidget(
                              readings: model.yesterdayGyroscope,
                              label: 'Yesterday',
                               title: 'Yesterday Gyroscope Readings',
                              dateType: 'Yesterday')
                          : Container();
                    }
                  } else {
                ...
                builder: (context, model, child) {
                  Widget content = Center(
                      child: Text(
                          'Error fetching data. Check your Internet connection'));
                  if (model.pastWeek) {
                    if (model.isPastWeekGyroscopeLoading) {
                      print(model.pastWeekGyroscope);
                      content = ShimmerLoader();
                    } else if ((model.pastWeekGyroscope.length == 0 &&
                        !model.isPastWeekGyroscopeLoading)) {
                      content = Center(
                          child: Padding(
                        padding: const EdgeInsets.all(8.0),
                        ...
                        !model.isPastWeekGyroscopeLoading)) {
                      content = GraphWidget(
                        readings: model.pastWeekGyroscope,
                        label: 'Past Week',
                         title: 'Past Week Gyroscope Readings',
                        dateType: 'Past Week',
                      );
                    }
                  } else {
                ...
                builder: (context, model, child) {
                  Widget content = Center(
                      child: Text(
                          'Error fetching data. Check your Internet connection'));
                  if (model.pastMonth) {
                    if (model.isPastMonthGyroscopeLoading) {
                      print(model.pastMonthGyroscope);
                      content = ShimmerLoader();
                    } else if ((model.pastMonthGyroscope.length == 0 &&
                        !model.isPastMonthGyroscopeLoading)) {
                      content = Center(
                          child: Padding(
                        padding: const EdgeInsets.all(8.0),
                        ...
                        !model.isPastMonthGyroscopeLoading)) {
                      content = GraphWidget(
                          readings: model.pastMonthGyroscope,
                          label: 'Past Month',
                        title: 'Past Month Gyroscope Readings',
                          dateType: 'Past Month');
                    }
                  } else {
                    content = Container();
                ...
                InkWell(
                  ...
                  onTap: () {
                    Navigator.push(
                        context,
                        CupertinoPageRoute(
                            builder: (context) => ExpandedGyroscope(
                              title: title,
...
class ExpandedGyroscope extends StatefulWidget {
  final List<SensorReading> readings;
  final String title;
  final String dateType;
  ExpandedGyroscope({this.readings, this.dateType, this.title});
  @override
  _ExpandedGyroscopeState createState() => _ExpandedGyroscopeState();
}

class _ExpandedGyroscopeState extends State<ExpandedGyroscope> {
  TrackballBehavior _trackballBehavior;
    ChartSeriesController _chartSeriesController;
